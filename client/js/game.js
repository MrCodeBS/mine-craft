class MinecraftGame {
    constructor() {
        ...
        const giantMaterial = new THREE.MeshBasicMaterial({ color: 0x00ff00, wireframe: true });
        ...
        console.log('Total objects in scene after test cubes:', this.scene.children.length);
    ...
    setupSocketEvents() {
        this.socket.on('gameState', (data) => {
            ...
            // Add welcome message
            this.addChatMessage('System', 'Welcome to Minecraft! CLICK THE CANVAS FIRST to enable movement!', '#ff0');
            this.addChatMessage('System', 'Use WASD to move, mouse to look around.', '#0ff');
            this.addChatMessage('System', 'Left click to destroy blocks, right click to place them.', '#0ff');
        });
        ...
        this.socket.on('playerJoined', (player) => {
            ...
            this.updatePlayerList();
            this.addChatMessage('System', `${player.username} joined the game`, '#0f0');
        });
        
        this.socket.on('playerLeft', (playerId) => {
            if (this.players[playerId]) {
                this.addChatMessage('System', `${this.players[playerId].username} left the game`, '#f00');
        ...
        this.socket.on('blockPlaced', (block) => {
            ...
            document.getElementById('blockCount').textContent = Object.keys(this.blocks).length;
        });
    ...
    createBlockMeshes() {
        ...
        console.log('Created', meshCount, 'block meshes out of', Object.keys(this.blocks).length, 'total blocks');
    ...
    createBlockMesh(block, key) {
        ...
        if (Object.keys(this.blockMeshes).length <= 3) {
            console.log(`Created block mesh at (${block.x}, ${block.y}, ${block.z}) type: ${block.type}`);
        }
    }
    ...
    updatePlayerMesh(player) {
        ...
        if (!this.playerMeshes[player.id]) {
            ...
            this.playerMeshes[player.id].position.set(player.x, player.y + 0.9, player.z);
        }
    }
    ...
    setupInput() {
        ...
        document.addEventListener('keydown', (e) => {
            switch (e.code) {
                ...
                case 'Space': this.controls.up = true; e.preventDefault(); break;
        ...
        this.renderer.domElement.addEventListener('click', () => {
            console.log('Canvas clicked, pointer locked:', this.isPointerLocked, 'chat visible:', this.chatVisible);
        ...
        this.renderer.domElement.addEventListener('mousedown', (e) => {
            ...
            if (e.button === 0) { // Left click - destroy
                ...
                this.socket.emit('destroyBlock', { x: blockData.x, y: blockData.y, z: blockData.z });
    ...
    update(deltaTime) {
        ...
        if (Object.values(this.controls).some(val => val)) {
            console.log('Controls active:', this.controls, 'Pointer locked:', this.isPointerLocked);
        }
        
        if (!this.isPointerLocked || this.chatVisible) return;
        
        // Movement with better speed
        const moveSpeed = 15 * deltaTime; // Increased speed
        ...
        if (moveDirection.length() > 0) {
            ...
            if (!this.lastServerUpdate || Date.now() - this.lastServerUpdate > 50) { // 20fps updates
        ...
        if (!this.lastUIUpdate || Date.now() - this.lastUIUpdate > 100) { // 10fps updates
    ...
    render() {
        ...
        if (this.renderCount <= 3) {
            console.log(`Render #${this.renderCount} - Camera pos:`, this.camera.position, 'Scene children:', this.scene.children.length);
        }
    ...
    gameLoop() {
        ...
        if (!this.lastFPSUpdate || Date.now() - this.lastFPSUpdate > 500) { // Every 0.5 seconds
